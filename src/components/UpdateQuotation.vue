  ...
  <div>
    ...
    <v-stepper v-model="step">
      <v-stepper-header>
        ...
        <v-divider></v-divider>
        <v-stepper-step step="2" :complete="step > 2">更改報價單內容</v-stepper-step>
        <v-divider></v-divider>
  ...
  export default {
    ...
    methods: {
      async retrieveQuotationList () {
        ...
        let failure = false
        try {
          let result = await service.Get('List/QuotationForSupplier/1')
      ...
      */
        ...
        if (changes && (changes[0][2] !== changes[0][3])) {
          ...
          try {
            ...
            if (result.status === 204) {
              let items = this.selectedQuotation[0].Items
      ...
      async completeQuotation () {
        console.log('Test Complete Quotation!')
        ...
        if (valid) {
          ...
          let param = {
            ...
            PrintHouseName: quotation.PrintHouseName,
            PurchaseDate: quotation.PurchaseDate,
            QuotationDeadline: quotation.QuotationDeadline,
            UpdateDate: quotation.UpdateDate,
            Items: null
          }
          try {
            let result = await service.Put('Quotation', param)
            if (result.status === 204) {
              this.step = 3
            } else {
              failure = true
